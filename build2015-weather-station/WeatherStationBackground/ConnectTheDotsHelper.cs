        ...
        private async Task<string> SendMessage(string message)
        {
            ...
            if (this.EventHubConnectionInitialized)
            {
                try
                {
                    ...
                    {
                        result = string.Format("Message Sent: {0}", content);                        
                    }
                    else
                    {
                        result = string.Format("Failed sending message: {0} {1}", postResult.ReasonPhrase, content);
                    }                    
                }
                catch (Exception e)
                {
                    result = "Exception when sending message:" + e.Message;
                }
